...
6. ⭐ **Nulla függőség — és a méret ott számít, ahol UTAZIK.** *(Szétválasztva 2026-09-06, Csaba döntése: „**az adat-csomagnak** kell kicsinek lennie. A telepítendő programnak nem kell kicsinek lennie.")*
   ...
   - 🟡 **LÁGY: a program mérete.** Ma **123 fájl, 1564,9 KB** — ⚠️ *ebből a `felulet/` 57 fájl / 408 KB, ami 2026-09-06-án érkezett: **örökölt, változatlan** kártya-kód és CSS a prototípusból (5.3).* Nem korlát, de érték: ekkora program **elfér egy üzenetben, és bárki újraírhatja** — ez a fojtópont-védelem másik fele. A felülettel (Szakasz 5) nőni fog, és **ez rendben van**; a szám itt attól hasznos, hogy tudjuk, hol tartunk.
...
node koino/meres/mind.js         # a 379 önpróba
...
📱 **Telefonra telepítés (Termux + Node):** [`docs/telepites_telefon.md`](docs/telepites_telefon.md) — a Szakasz 2 / 4. lépéséhez. `git clone --depth 1` a nyilvános repóból (5,6 MB a 23 helyett). A `koino/` mappa **önmagában futtatható**: 123 fájl, 1564,9 KB (a `tar.gz` csomag ~80 KB), nulla függőség — *ugyanaz a szám, mint a 6. szabálynál; ha az egyik változik, mindkettőt vezesd át.* ⚠️ A mércét a 6. szabály mondja meg: **bájtok összege, nem `du`**.
...
- **Önpróbák:** `node koino/meres/mind.js` — 379 próba tizenhárom fájlban; a kilépési kód 1, ha bármi bukott. Egy réteg külön is: `node koino/meres/mind.js szabaly`. ⚠️ A szűrő **részszóra** illeszkedik: a `tar` a `tarsak`-ot is elindítja (13 + 26 = 39) — ez nem hiba, de a próbaszám olvasásakor félrevezet. Nincs teszt-könyvtár. A koino részletes naplója alapból néma, `KOINO_NAPLO=1`-gyel kapcsolható be.
...
- `js/allapot/egyezmenyVegrehajtas.js` — ⭐ **A HARMADIK FÁZIS** (2026-09-06): az elfogadott **szerkesztési** egyezmények rávezetése az entitásokra. A **D8** gyakorlati alakja: az egyezmény a TÉNY, az entitás mai alakja a HATÁLY. ⚠️ Az **általános** egyezményből soha nem következik entitás-változás (D27). A sorrend a **lejárat** szerint dől el (holtversenynél az azonosító), sosem a térkép bejárása szerint — különben két gép mást mutatna. Amit nem tud végrehajtani, azt **felsorolja** (`kihagyottak`), nem hallgatja el (D19).
