...
**Olvasd el induláskor:** ⭐ [`docs/utiterv.md`](docs/utiterv.md) — **mit építünk, milyen sorrendben, és miért** (rövid; ez a belépő). Utána: [`docs/fejlesztesi_terv_fazis2.md`](docs/fejlesztesi_terv_fazis2.md) → az elején a **„HOL TARTUNK"** szakasz. A szakasz-tervek: [`docs/szakasz1_terv.md`](docs/szakasz1_terv.md) (**1. A helyi modell** — ✅ kész) · [`docs/szakasz2_terv.md`](docs/szakasz2_terv.md) (**2. A szállítás** — ✅ kész; a neve eddig „csere" volt, de a munka a szállításról szólt). ✅ **3. A szerkezet** — kész (2026-09-03): a kanonikus alak négy új mezője, a kérdezhető tár-illesztő, a böngésző-lekérés. 🚧 **Most a 4. szakasz folyik: AZ IDENTITÁS** — terv: [`docs/szakasz4_terv.md`](docs/szakasz4_terv.md) (tervezés; **kód még nincs**). ⭐ **A megépítés terve a 9/b szakaszban:** tíz lépés (4.0–4.9) — a **4.1–4.4 a LÁNC** (kemény mag, offline dönt), a **4.5–4.8 a MÁSODIK RÉTEG** (jelzés és bemutatkozás, élő és **elhagyható**), a 4.9 a `lancGyoker` lefoglalt helye. ⚠️ **Három döntés vár Csabára, addig ne írj kódot:** **D51** — a `k` paraméter, nem méret-küszöb (`k = 0` a kis koinóban), mert a „mekkora méret fölött" maga is globális szám · **D52** — a tanúsítás **a tanúsított szeletébe** kerül (ettől lesz a „kapott" oldal korlátos lekérdezés) · ⚠️ **D53** — a keret **a kiadás oldalán kemény, a befogadás oldalán jelzés** (ez oldja fel a rekurziót, és ez a legfontosabb eldöntetlen pont). ⚠️ *A D17 szerint enélkül minden eddigi munka egy hatékonyan skálázódó hamisítás-gépezet.* ⭐ **A keret-szabály áll** (D44): *annyi tanúsítást adhatsz, amennyivel a belépési minimum fölött vagy.* ⭐⭐ **A FORDULAT (D49):** az identitás-réteg **egy minimumon kívül nem tilt, hanem FELTÁR** — a jelzés **tényt mutat, soha nem ítéletet**, és a közösség dönt. ⭐⭐⭐ **ÉS A SATU:** a személyes statisztika a **ritka** hamis szigetet fogja meg, a **bemutatkozások tengere** (véletlen séták: összeérnek-e?) a **sűrűt** — a támadó nem lehet egyszerre mindkettő. Mérve: **100% / 0%** mind a három szimulált támadó ellen, helyi és korlátos számítással. ⭐⭐ **És a bemutatkozás nem lánc-esemény, hanem HELYI LISTA** (D50) — mint a `tarsak.js` társ-listája —, a séta pedig **ugráló üzenet**, nem lekérdezés: *a jelzéshez nem kell a térkép, csak az út.* A tenger így a **második rétegbe** kerül (élő, elhagyható), nem a DAG-ba. ⚠️ **Négy zsákutca, amit ne javasolj újra** (mind megmérve, [`koino/meres/eredmenyek.md`](koino/meres/eredmenyek.md)): a Duniter-féle távolság-szabály **globális számokat kíván** · az „ingyenes elismerés" a szigetet ugyanúgy táplálja · **a gazdaság önmagában nem véd** · a horgony-kört a támadó **elfoglalja** (880 hamis horgony), mert a „több független körből" feltétel a **tudatos hálózatépítőt** jutalmazza. ⛔ **És ne javasold azt sem, hogy „kevés kapcsolata van, tehát gyanús"** — háromszor mérve 31/41/45% becsületes tagot jelölne meg tévesen, épp a magányost és a frissen érkezettet. A tervezési döntések (**D1–D50**, a D48 elvetve) is ott vannak — a Szakasz 4 döntései 2026-09-05-én átvezetve. A milliárdos lépték szerkezete: [`docs/skalazas_terv.md`](docs/skalazas_terv.md) (2026-08-31 — tervjavaslat, kilenc döntést igénylő ponttal). **Az irány két réteg:** a **DAG** a hitelességé és offline is működik · a **kereső-réteg** a megtalálhatóságé, hálózatot kíván, és **elhagyható**. ⭐ *Ami DÖNT valamiről, az soha ne kívánjon élő lekérdezést; csak a MEGTALÁLÁS kívánhat.*
...
6. ⭐ **Nulla függőség, kis méret.** Ma **35 fájl, 612 KB** a mappa (tömörítve ~80 KB), **0 npm-csomag**. Ez **védelem, nem elegancia**: elfér egy üzenetben, és bárki újraírhatja. Minden új függőség egy újabb fojtópont. ⚠️ *Ha új fájl kerül a `koino/`-ba, ezt a számot itt is vezesd át — a szabály attól ellenőrizhető, hogy a mércéje friss.*
...
node koino/meres/mind.js         # a 217 önpróba
...
📱 **Telefonra telepítés (Termux + Node):** [`docs/telepites_telefon.md`](docs/telepites_telefon.md) — a Szakasz 2 / 4. lépéséhez. `git clone --depth 1` a nyilvános repóból (5,6 MB a 23 helyett). A `koino/` mappa **önmagában futtatható**: 35 fájl, 612 KB (a `tar.gz` csomag ~80 KB), nulla függőség — *ugyanaz a szám, mint a 6. szabálynál; ha az egyik változik, mindkettőt vezesd át.*
...
- **Önpróbák:** `node koino/meres/mind.js` — 217 próba kilenc fájlban; a kilépési kód 1, ha bármi bukott. Egy réteg külön is: `node koino/meres/mind.js szabaly`. ⚠️ A szűrő **részszóra** illeszkedik: a `tar` a `tarsak`-ot is elindítja (13 + 26 = 39) — ez nem hiba, de a próbaszám olvasásakor félrevezet. Nincs teszt-könyvtár. A koino részletes naplója alapból néma, `KOINO_NAPLO=1`-gyel kapcsolható be.
